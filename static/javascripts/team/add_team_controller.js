...
app.controller('addTeamController', ["$scope", "$stateParams", "$uibModal", "teamService", "toastr", function($scope, $stateParams, $uibModal, teamService, toastr) {
	var self = this;
	$scope.self = self;
	self.showAddModal = function(team){
	      ...
	      resolve: {
	        ...
	        fields: function () {
				...
				return [{name: "Name", value: name, type: "text", var_name: "name"},
						{name: "Days of Week", value: days_of_week, type: "checkbox", var_name: "days_of_week", options: 
						...
						{name: "Report Summary Time", value: summary_send_time, type: "timepicker", var_name: "summary_time"}];
						}
	      }
	    });
